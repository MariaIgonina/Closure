...
function ChartDens() {
  ...
  const { densityJSON } = useContext(Context)

  const [ chartData, setchartData ] = useState([])

  useEffect(() => {
    if (Object.keys(densityJSON).length !== 0) {
      const keys = densityJSON.labels
      const values = densityJSON.values
      const converted = []
